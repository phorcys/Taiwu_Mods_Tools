...
namespace UnityModManagerNet.Installer
{
    public partial class UnityModManagerForm : Form
    {
        ...
        private void SaveAndInstallZipFiles(string[] files)
        {
            ...
            foreach (string filepath in files)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Log.Print($"安装zip文件时发生错误，文件： '{Path.GetFileName(filepath)}'.");
                }
            }
            ...
            if (newMods.Count > 0)
            {
                foreach (var modInfo in newMods)
                {
                    ...
                    while (tempList.Count > 2)
                    {
                        ...
                        {
                            Log.Print(ex.Message);
                            Log.Print($"无法删除临时文件路径 '{item.Path}'.");
                            break;
                        }
                    }
                }
        ...
        private void InstallMod(string filepath)
        {
            ...
            {
                Log.Print($"文件无法找到： '{Path.GetFileName(filepath)}'.");
            }
            try
            {
                using (var zip = ZipFile.Read(filepath))
            ...
            {
                Log.Print(e.Message);
                Log.Print($"安装失败： '{Path.GetFileName(filepath)}'.");
            }
        }
        ...
        private void InstallMod(ZipFile zip, bool reloadMods = true)
        {
            ...
            try
            {
                ...
                Log.Print($"解压 '{Path.GetFileName(zip.Name)}' - 成功.");
            }
            catch (Exception ex)
            {
                Log.Print(ex.Message);
                Log.Print($"解压错误： '{Path.GetFileName(zip.Name)}'.");
            }
